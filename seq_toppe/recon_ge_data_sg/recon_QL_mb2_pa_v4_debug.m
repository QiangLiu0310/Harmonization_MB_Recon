...
kspace=reshape(kspace,[size(kspace,1), size(kspace,2), size(kspace,3), size(kspace,4)/2, 2]);
sli_idx=[1:2:NSlc 2:2:NSlc];
[~,or]=sort(sli_idx);
kspace=kspace(:,:,:,or,:);

rowFlippedKSpace = zeros(330,146,44,90);
phaseCorrectedKSpace = zeros(330,146,44,90);
interpolatedData = zeros(146,146,44,90);
interpolatedData_final = zeros(146,146,44,90,2);

for imulti = 1:2
    for geometricSliceNumber = 1:90
        % Use parfor for the innermost loop
        for channel = 1:44
            kSpaceWithoutRefViews = kspace(:,:,:,:, imulti);
            % Perform operations in parallel for each channel
            rowFlippedKSpace(:,:,channel,geometricSliceNumber) = GERecon('Epi.ApplyImageRowFlip', rowFlipHandle, squeeze(kSpaceWithoutRefViews(:,channel,:,geometricSliceNumber)));
            phaseCorrectedKSpace(:,:,channel,geometricSliceNumber) = GERecon('Epi.ApplyPhaseCorrection', phaseCorrectionHandle, rowFlippedKSpace(:,:,channel,geometricSliceNumber), geometricSliceNumber, channel);
            interpolatedData(:,:,channel,geometricSliceNumber) = GERecon('RampSampling.Interpolate', vrgfHandle, phaseCorrectedKSpace(:,:,channel,geometricSliceNumber));
        end
    end
    interpolatedData_final(:,:,:,:,imulti) = interpolatedData;
end

GERecon('Archive.Close', archive);

coil=zeros(146,146,44,90);
coil(:,1:2:end,:,:)=interpolatedData_final(:,1:2:end,:,:,1);
coil(:,2:2:end,:,:)=-interpolatedData_final(:,1:2:end,:,:,2);
coil=coil(end:-1:1,:,:,:);
...
if ~exist('Npppp','var')
    ...
    for slc = 1:NSlc;
        fprintf('o');
        [~,~,Np{slc}]=recongrappa_multik(sz([2 1 4]),permute(k_trgt(:,:,slc,:),[2 1 4 3]),[],'kernel','2x5','dks',hdr.R*[1 2],... %2*5
            'chi',chi,'eta',eta );
    end;
    fprintf('\n');
...
cd /data/pnlx/home/ql087/data_processing/2024_recon_reproducibility/sub5/ge/scan1/split_grappa/
