function snr = mrir_array_SNR_cov_incr(img_uncombined, Rn)
%MRIR_ARRAY_SNR_COV_INCR
%
% snr_incr = mrir_array_SNR_cov_incr(img_uncombined, Rn)

% jonathan polimeni <jonp@nmr.mgh.harvard.edu>, 2008/apr/28
...
%**************************************************************************%
  ...
  snr_incr_reshape = cumsum(snr_reshape, 2);


  snr = sqrt(abs( reshape(snr_incr_reshape.', [Ncha, Ncol, Nlin, dims(4:end)]) ));
  snr = ipermute(snr, [3, 1, 2, 4:ndims(img_uncombined)]);
