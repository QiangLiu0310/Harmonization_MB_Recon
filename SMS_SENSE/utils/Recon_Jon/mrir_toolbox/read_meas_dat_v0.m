...
%%%% Kawin: add infor about order of slice position for interleave case 6/21/09
% testFlag = [];
% counter = 1;
SlicePos = [];
IceParaTotal1 = [];
...
%%%%
  ...
  if ( str2num(matlab_version(1:3)) < 7.2 ) && matlab_version(4) == '.'  % Kawin Setsompop (make it work for version 7.10 and up)
...
%              pos(10), pos(11), pos(12), pos(13), pos(14), pos(15), pos(16)) = mdh(idx).ulScanCounter;
          ...
          if ( DO__RETURN_STRUCT )
              if pos(02) == 1 && pos(03) == 1 % i.e. only do for line1, chn1,
                 % IcePara = mdh(idx).aushIceProgramPara(1:MDH_NUMBEROFICEPROGRAMPARA);
                 % IceParaTotal1 = [IceParaTotal1; IcePara];
                 % IceParaTotal2 = [IceParaTotal2; ((IcePara-1)/10000 -1)];
                  if pos(07) == 1 % only do for rep1
                      SlicePos = [SlicePos; mdh(idx).flTra];
                      
                      %                   if size(SlicePos) ~= 0
                      %                       if sum(SlicePos ~= mdh(idx).flTra) == length(SlicePos) % i.e. has not repeated
                      %                           SlicePos = [SlicePos; mdh(idx).flTra];
                      %                       end
                      %                   else
                      %                       SlicePos = mdh(idx).flTra;
                      %                   end
                  end
              end
          end
