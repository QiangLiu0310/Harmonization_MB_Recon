...
% $Id: read_meas_prot.m,v 1.8 2008/12/01 03:25:21 jonnyreb Exp $
%**************************************************************************%
  ...
  for slc = 1:YAPS.sSliceArray_lSize,
    ...
    % %     Jon & Kawin : make it able to detect value that is very small that get display as
    % %     scientific number in the header e.g. 5e-16
  ...
  if ( ~isempty(Config_evp.NColMeas) ),
    ...
    for contrast = 1:YAPS.lContrasts,
      ...
      YAPS.flBandwidth_derived(contrast) = bandwidth;
  ...
  %************************************************************************%
  %%% $Source: /space/padkeemao/1/users/jonnyreb/cvsjrp/PROJECTS/IMAGE_RECON/mrir_toolbox/read_meas_prot.m,v $
  %%% Local Variables:
  %%% mode: Matlab
  %%% fill-column: 76
  %%% comment-column: 0
