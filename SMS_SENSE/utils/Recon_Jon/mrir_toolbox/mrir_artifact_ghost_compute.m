...
if nargin == 1
    PercentDataUse = 0.5; % 0.4 
    RegressionMethod = 1; % 2
    TakeMedian = 1; % use 2 to do moving median
elseif nargin == 2
    RegressionMethod = 1;
    TakeMedian = 1;
end
...
%   %% debugging plot
%   linear_fit_coeff1 =   reshape(linear_fit_coeff(1,:), [1 1 dims(3:7) 1 1 dims(10)]);
%   linear_fit_coeff2 =   reshape(linear_fit_coeff(2,:), [1 1 dims(3:7) 1 1 dims(10)]);
%   
%   for repCount = 1:dims(7)
%       for slcCount = 1:dims(10)
%           figure(100+slcCount); subplot(2,5,repCount); plot(reshape(linear_fit_coeff1(1,1,:,:,:,:,repCount,1,1,slcCount), [],1)); ylim([-1.5 1.5]);
%            subplot(2,5,repCount+5); plot(reshape(linear_fit_coeff2(1,1,:,:,:,:,repCount,1,1,slcCount), [],1)); ylim([-0.3 0.3]);
%       end
%   end
%  
%   
%   if dims(7) < 10 % assume no time varying component and take mode
%       
%   else   
%         var(linear_fit ,0, 
%   end
%   
%   mean_coeff1 = mean(linear_fit_coeff1,7);  
%   mean_coeff_slc1 =  mean(mean_coeff1,10);
