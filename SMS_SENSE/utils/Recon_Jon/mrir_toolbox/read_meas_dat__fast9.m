...
% $Id: read_meas_dat__fast8.m,v 1.1 2009/10/02 19:45:45 jonnyreb Exp $
%**************************************************************************%
...
%  scan_patrefscan          = matrixtype([]);
  ...
  try,
    ...
    while ( ~ACQEND ),
      ...
      [mdh, scan_num] = read_meas_dat__mdh_binary__alt(mdh_binary);
...
%%%        end;
      ...
      elseif ( read_meas__extract_bit(mdh(idx).aulEvalInfoMask(1), EvalInfoMask.MDH_PATREFSCAN) ),
        ...
        if ( pos(16) > acq_counter ),
          if ( ~FLAG__repeated_patrefscan ),
            disp('  multiple PATREFSCAN acquistions detected.');
            FLAG__repeated_patrefscan = 1;
          end;

          disp(sprintf('   acq: %02d', acq_counter));
          acq_counter = pos(16);
        end;
...
%    disp(lasterr);
    ...
    if ( feof(fp) ),
      ...
      if ( 0 ),
        ...
        disp(['??? ', lasterr]); beep; pause(0.1); beep;
        keyboard
      end;
...
% //##ModelId=3AFAAF7801CF
  ...
  %************************************************************************%
  %%% $Source: /space/padkeemao/1/users/jonnyreb/cvsjrp/PROJECTS/IMAGE_RECON/mrir_toolbox/read_meas_dat__fast8.m,v $
  %%% Local Variables:
  %%% mode: Matlab
  %%% fill-column: 76
  %%% comment-column: 0
