...
function [meas] = read_meas_dat_memmap_v2_VD11_readall(filename,ascendingSlice_acq,Data7T)
...
%% VD 11 HACK	
	coilIndexMask = squeeze(sum(meas_first.data(floor(end/2),floor(end/2),:,1,1,1,1,:,1,1),8)) ~=0;
	meas_first.data = meas_first.data(:,:,coilIndexMask,:,:,:,:,:,:,:);
	meas_first.data_phascor1d = meas_first.data_phascor1d(:,:,coilIndexMask,:,:,:,:,:,:,:);
	if isfield(meas_first, 'patrefscan')
		meas_first.patrefscan = meas_first.patrefscan(:,:,coilIndexMask,:,:,:,:,:,:,:);
		meas_first.patrefscan_phascor = meas_first.patrefscan_phascor(:,:,coilIndexMask,:,:,:,:,:,:,:);
    end
...
%% 
nRep = meas.evp.NSetMeas+meas.evp.NRepMeas-1
meas.evp.NRepMeas = nRep;

sData = [meas.evp.NColMeas, meas.evp.NLinMeas, meas.evp.NChaMeas,...
          1, 1, 1, nRep, 2, 1, meas.evp.NSlcMeas];
...
disp('Mmap')
tic
m = mmap_mdh_noheader_rep_VD11(filename,linesperrep,nRep,nRead,nCoil,meas.data_begin);
...
for RepCount = 1:meas.evp.NRepMeas
    disp([ 'Reading in Rep:' num2str(1+RepCount-1) ])
    tic
    k = m.Data(1+(RepCount-1)).kdata;
    
    %k = reshape(k,[],linesperrep)*K_ICE_AMPL_SCALE_FACTOR;
    %k = k(33:2:end,:)+i*k(34:2:end,:); %32 headers
    
    PElinesPerRep = linesperrep/nCoil;
    kPE = reshape(k,[],PElinesPerRep)*K_ICE_AMPL_SCALE_FACTOR; 
    k = kPE(mdh_length_float32+1:end,:); 
    k = reshape(k,[],linesperrep);
    k = k(mdh_ch_length_float32+1:2:end,:)+i*k(mdh_ch_length_float32+2:2:end,:); 

    k = reshape(k, [nRead, nCoil, nPE_Raw+nPhaseCor, nSlice]);    
    
%     figure;
%     for count = 1:10
%         subplot(2,5,count); imagesc(squeeze(abs(k(:,:,count,1))),[0 10])
%     end
%     
