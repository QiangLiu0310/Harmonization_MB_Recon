function [epi_img, epi_raw] = mrir_epi(meas,PercentDataUse,RegressionMethod)
%MRIR_EPI
%
...
%**************************************************************************%
  ...
  if ( nargin == 1 ), 
      PercentDataUse = 0.5;  % use 50% of data for ghost computation
  end;
  ...
  if nargout == 2
      epi_raw    = complex(zeros(dims), zeros(dims));
  end
  dims(1) = dims(1) / 2;
  epi_img     = complex(zeros(dims), zeros(dims));
...
%         end;
      ...
      linear_fit_coeff = mrir_artifact_ghost_compute(meas.data_phascor1d(:,:,:,1,1,1,rep,:,1,slc),PercentDataUse,RegressionMethod);
