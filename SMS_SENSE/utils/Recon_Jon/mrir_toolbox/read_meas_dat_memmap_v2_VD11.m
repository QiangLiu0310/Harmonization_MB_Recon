...
function [meas] = read_meas_dat_memmap_v2_VD11(filename,nRepToRead,BeginRep,ascendingSlice_acq,Data7T)
...
%% VD 11 HACK: cant reorder coil as each ID corresponse to 2 channels
...
%% VD 11 HACK	
	coilIndexMask = squeeze(sum(sum(meas_first.data(floor(end/2),1:end,:,1,1,1,1,:,1,1),8),2)) ~=0;
