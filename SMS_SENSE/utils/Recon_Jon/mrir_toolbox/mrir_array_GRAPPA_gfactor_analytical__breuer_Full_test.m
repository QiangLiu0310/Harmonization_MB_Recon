function [covmtxFull, g, g_ind, n, n_ind] = mrir_array_GRAPPA_gfactor_analytical__breuer_Full_test( w, KernalSize_Slice, Phase, G, R_inplane, covmtx, NCol, NLin, varargin)
        ...
        for col = 1:size(K,1),
            ...
            if nargout > 1
                ...
                if combineMethod == 1
                    ...
                    %p = squeeze(combine_weights(col,lin,:,1,1,1,1,1,1,slc)).' * inv(squeeze(covmtxFull(col, lin, :,:,slc)));
                    p = squeeze(combine_weights(col,lin,:,1,1,1,1,1,1,slc)).' / squeeze(covmtxFull(col, lin, :,:,slc));
                    p2 = squeeze(combine_weights(col,lin,:,1,1,1,1,1,1,slc)).' * i_noisecov_INV;
                    p = p/(p*squeeze(conj(combine_weights(col,lin,:,1,1,1,1,1,1,slc))));
                    p2 = p2/(p2*squeeze(conj(combine_weights(col,lin,:,1,1,1,1,1,1,slc))));
                    n(col, lin, slc) = sqrt(abs( p * squeeze(covmtxFull(col, lin, :,:,slc)) * p' ));
                    g(col, lin, slc) = n(col, lin, slc) ./ sqrt( abs(p2 * i_noisecov * p2') ) /sqrt(R_inplane); 
                end
