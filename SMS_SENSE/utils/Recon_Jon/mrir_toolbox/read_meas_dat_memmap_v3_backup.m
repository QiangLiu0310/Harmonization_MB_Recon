...
function [meas] = read_meas_dat_memmap_v3(filename,nRepToRead,BeginRep,SMSdata,ascendingSlice_acq)
...
%     if opt.SMSRefscan == 1
%         [meas_first.prot.SMS_factor,SlcMask] = SMSfactorFind(meas_first);
%     else
%         meas_first.prot.SMS_factor = 1;
%         SlcMask = 1:size(meas_first.data,10);
%     end
    if isfield(meas_first, 'patrefscan')
        ...
        b = meas_first.data(1,:,1,1,1,1,1,1,1,index(1));
        b = squeeze(b);
        ...
        c = meas_first.data(1,:,1,1,1,1,1,2,1,index(1));
        c = squeeze(c);
...
sData = [meas.evp.NColMeas, meas.evp.NLinMeas, meas.evp.NChaMeas,...
          1, 1, 1, nRepToRead, 2, 1, meas.evp.NSlcMeas/meas.prot.SMS_factor];
...
function [SMS_factor,SlcMask] = SMSfactorFind(meas_first)
