...
function [meas] = read_meas_dat_memmap_v3_2_buda(filename,nRepToRead,BeginRep,SMSdata,ascendingSlice_acq,data_isvd,AccZ)
...
if exist([save_fname(1:end-4) '_Raw.mat'],'file') ~= 2
    ...
    meas_first.evp.NColMeas  = size(meas_first.data,1);   % CL: hack the colmeas 
    
%     meas_first.evp.RawLin ;
%     meas_first.evp.RawRep;
    disp('*********************************************')
