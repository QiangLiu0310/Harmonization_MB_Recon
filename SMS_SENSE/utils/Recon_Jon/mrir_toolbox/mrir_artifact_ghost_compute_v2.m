function linear_fit_coeff = mrir_artifact_ghost_compute_v2(phascor)
...
% Kawin Setsompop, 2010/april/06
% chage criterion on which part of the projected data will be use to calc
% the ghost term. This will be base on amplitude of signal rather than
% truncation
...
%   projections = mrir_image_crop(projections);
  ...
  % [       1 x Nlines]
  phasediff_undersample = phasediff(1:2:end,:);
  phasediff_undersample2 = unwrap(phasediff_undersample, [], 1);
  
  phase_correction = phasediff_undersample2 - phasediff_undersample;
  mask = zeros(size(phasediff));
  mask(1:2:end,:) = diff(phase_correction) > 1;
...
%   
%   
%   phasediff_center_pos = (mean(phasediff, 1) > +pi) .* 2*pi .*ceil( mean(phasediff, 1)/ (2*pi) );
%   % [Nreadout x Nlines]
%   phasediff_offset_pos = repmat(phasediff_center_pos, size(phasediff,1), 1);
% 
%   phasediff_center_neg = (mean(phasediff, 1) < -pi) * 2*pi .*floor( mean(phasediff, 1)/ (2*pi) );
%   phasediff_offset_neg = repmat(phasediff_center_neg, size(phasediff,1), 1);
%   
%   phasediff = phasediff - phasediff_offset_pos;
%   phasediff = phasediff - phasediff_offset_neg;
