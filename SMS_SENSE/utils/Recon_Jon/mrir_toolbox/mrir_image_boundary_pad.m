...
%**************************************************************************%
  ...
  mask_frame_init = padarray(mask_init, Nframe*[1 1], 0, 'both');
  ...
  mask_final = imerode(mask_close, strel('disk', pixel_trim));
  ...
  for iteration = 1:pixel_pad,
    ...
    current_size = size(img_iter);

    img_re_expand = imresize(real(img_iter), current_size+2, 'bilinear');
    img_im_expand = imresize(imag(img_iter), current_size+2, 'bilinear');
    img_expand = complex(img_re_expand, img_im_expand);

    
    img_grow = padarray(img_chan, [1 1]*iteration, 0, 'both');

    mask_grow = padarray(mask_grow, [1 1], 0, 'both');
  ...
  img_re_pad = imcrop(real(img_iter), [pixel_pad+1, pixel_pad+1, size(img,1)-1, size(img,2)-1]);
  img_im_pad = imcrop(imag(img_iter), [pixel_pad+1, pixel_pad+1, size(img,1)-1, size(img,2)-1]);
