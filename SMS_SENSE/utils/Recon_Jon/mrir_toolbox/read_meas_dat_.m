function varargout = read_meas_dat(wildfile, user_options, varargin)
%READ_MEAS_DAT  read in Siemens format raw data, VB13/VB15/VB17-style "meas.dat"
%
...
% $Id: read_meas_dat.m,v 1.11 2011/11/21 07:01:06 jonp Exp $
%**************************************************************************%
...
%  end;
  ...
  measfile = fullfile(pathstr, getfield(dir(wildfile), {1}, 'name'));
  ...
  % some sequences, like EPI with DORK, have useless navigators with
  % RTFeedback flag set and these lines should be skipped, whereaas other
  ...
  if ( exist('user_options', 'var') ),
    ...
    if ( isfield(user_options, 'RemapChannels') ),
      DO__REMAP_CHANNELS = user_options.RemapChannels;
      disp_optional(sprintf(' :REMAP_CHANNELS = %d', DO__REMAP_CHANNELS));
    end;
  ...
  options = struct('ExtractRepetition',             DO__EXTRACT_REPETITION, ...
                   'ReverseLines',                  DO__FLIP_REFLECTED_LINES, ...
                   ...
                   'RemapChannels',                 DO__REMAP_CHANNELS, ...
  ...
  if ( ~isempty(user_option_list) ),
    ...
    if ( ~all(user_option_list_VALID) ),
      ...
      for item = 1:length(invalid_items),
        warning(sprintf('option "%s" not recognized', strvcat(user_option_list{item})));
      end;
    end;
  ...
  elseif (  ( DO__PREALLOCATE || DO__CANONICAL_REORDER_COIL_CHANNELS || DO__RETURN_STRUCT ) && ~DO__EXTRACT_REPETITION  ),

    % read header into one string for parsing
    header = char(fread(fp, data_start-4, 'uchar').');
...
%  scan_patrefscan          = matrixtype([]);
  ...
  try,
    ...
    while ( ~ACQEND ),
      ...
      if ( meas_num == 1 ),
        mdh_first = mdh(1);
        meas_timestart = mdh(1).ulTimeStamp;
      end;
      ...
      if ( DO__EXTRACT_REPETITION ),
        ...
        if ( read_meas__extract_bit(mdh(idx).aulEvalInfoMask(1), EvalInfoMask.MDH_NOISEADJSCAN)  || ...
             read_meas__extract_bit(mdh(idx).aulEvalInfoMask(1), EvalInfoMask.MDH_PATREFSCAN)    || ...
        ...
        % positive once we've seen a line of data and recorded its position
        if ( firstdata_pos > 0 ),
          ...
          if ( bytes_per_rep < 0 ),
            ...
            bytes_per_rep = meas_pos - firstdata_pos;
      ...
      if ( read_meas__extract_bit(mdh(idx).aulEvalInfoMask(1), EvalInfoMask.MDH_REFLECT) ),
        if ( ~FLAG__data_reflect ),
          if ( DO__FLIP_REFLECTED_LINES ),
            disp_optional(' REFLECT detected, reversing lines.');
          else,
            disp_optional(' REFLECT detected.');
          end;
      ...
      if ( read_meas__extract_bit(mdh(idx).aulEvalInfoMask(1), EvalInfoMask.MDH_RTFEEDBACK) ),
        if ( ~FLAG__rtfeedback ),
          ...
          if ( DO__SKIP_RTFEEDBACK ),
            disp(' RTFEEDBACK detected, skipping all feedback lines.');
          else,
            disp(' RTFEEDBACK detected.');
          end;
        end;
        % if option is set, skip this line!
      ...
      if ( ~FLAG__stored_channel_indices ),
        if ( ~ismember(pos(03), channel_indices) ),
          ...
          if ( DO__REMAP_CHANNELS && ...
            ...
            if ( FLAG__smsrefscan ),         smsrefscan         = smsrefscan(           :,:,channel_sort); end;
      ...
      elseif ( read_meas__extract_bit(mdh(idx).aulEvalInfoMask(1), EvalInfoMask.MDH_PATREFSCAN) ),
        ...
        if ( read_meas__extract_bit(mdh(idx).aulEvalInfoMask(1), EvalInfoMask.MDH_PHASCOR) ),
          ...
          if ( ~FLAG__patrefscan_phascor ),
            ...
            % this shouldn't be necessary...but the MDH_FIRSTSCANINSLICE flag doesn't get set for ACS lines! grr...
            pat_nav_slice = 0;
...
%              pos(10), pos(11), pos(12), pos(13), pos(14), pos(15), pos(16)) = mdh(idx).ulScanCounter;
	...
	%%% CATEGORY #5: SMS reference data
	  ...
	  % the SMS reference isn't actually detected, but is defined as the
          % first repetition!
          disp_optional(' SMSREFSCAN chosen.');
          FLAG__smsrefscan = 1;
...
%%%%      end;
          ...
          %% [2011/sep/16] do not need these pesky OFFLINE lines any more; in future will
          %% add an option to allow user to request the OFFLINE scans.
...
%    meas = struct;
    ...
    meas.acqtime = [meas_timestart, mdh(end).ulTimeStamp];
