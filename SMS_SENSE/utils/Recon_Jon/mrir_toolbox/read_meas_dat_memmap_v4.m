...
function [meas] = read_meas_dat_memmap_v4(filename,nRepToRead,BeginRep,SMSdata,ascendingSlice_acq)
...
if exist([save_fname(1:end-4) '_Raw.mat'],'file') ~= 2
    ...
    if  (isfield(meas_first, 'patrefscan'))
%         meas_first.evp.NFirstRefPar = find(sum(meas_first.patrefscan(end/2,:,1,1,1,1,1,:,1,1),8),1); %MKM 
        if meas_first.evp.NFirstRefPar == 0 
        meas_first.evp.NFirstRefPar =1;
        end
...
%         s(2) = L1 + L2;
%         patrefscan_phascorTemp = zeros(s);
%         
...
%%% MKM - this section is kind of a mess. 
%%% depends on sequence special card layout 
meas = meas_first;
clear meas_first;
if SMSdata == 1
    if meas.isvd == 1    
        if ~isempty(meas.prot.sWiPMemBlock_adFree)
            ...
            meas.prot.sWiPMemBlock_adFree(2) = meas.prot.sWipMemBlock_adFree(3); % SMS factor
            meas.prot.sWiPMemBlock_adFree(1) = meas.prot.sWipMemBlock_adFree(2);%*meas.prot.lAccelFactPE % FOV shift factor
...
%             meas.prot.sWiPMemBlock_adFree(2) = 2; % shift in reduced FOV
            ...
            meas.prot.sWiPMemBlock_adFree(1) = prottemp.sWiPMemBlock_adFree(2); % SMS
            
        end
       % MKM meas.prot.sWiPMemBlock_adFree(3) = meas.prot.dThickness/meas.prot.sWiPMemBlock_adFree(1); % slice seperation in mm
        meas.prot.sWiPMemBlock_adFree(3) = meas.prot.dThickness/meas.prot.sWiPMemBlock_adFree(2); % slice seperation in mm      
...
linesperrep = (nPE_Raw*nEco+nPhaseCor)*nCoil*nSlice;
...
%     k = reshape(k, [nRead, nCoil, nPE_Raw+nPhaseCor, nSlice]);
    ...
    k = reshape(k, [nRead, nCoil, (nPE_Raw*nEco)+ nPhaseCor, nSlice]);
    ...
    k = reshape(k(:,:,nPhaseCor+1:end,:),nRead,nCoil,nPE_Raw,nEco,nSlice); % strip off the phase cor and arrange into multi-echo
  % if (isfield(meas, 'offline') && ~isfield(meas, 'patrefscan'))
