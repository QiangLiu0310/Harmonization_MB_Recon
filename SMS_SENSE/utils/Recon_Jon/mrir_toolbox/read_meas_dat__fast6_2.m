% change read_meas_dat__fast6 by adding Nmeas but doesnt seem to work....


function varargout = read_meas_dat(filename, user_options, varargin)
...
% $Id: read_meas_dat__fast.m,v 1.1 2009/09/20 00:57:33 jonnyreb Exp $
%**************************************************************************%
  ...
  if ( exist('user_options', 'var') ),
    ...
    if ( isfield(user_options, 'ExtractRepetition') ),
      ...
      disp(sprintf(' :EXTRACT_REPETITION = %d', DO__EXTRACT_REPETITION));
      iRep = varargin{1};
      Nmeas = varargin{2};
    end;
    ...
    if ( isfield(user_options, 'P hascorCollapseSegments') ),
    ...
    if ( isfield(user_options, 'RemapChannels') ),
      DO__REMAP_CHANNNELS = user_options.RemapChannels;
      disp(sprintf(' :REMAP_CHANNELS = %d', DO__REMAP_CHANNELS));
    end;
...
%%      end;
    ...
    if ( DO__APPLY_FFT_SCALEFACTORS && DO__EXTRACT_REPETITION ),
      DO__APPLY_FFT_SCALEFACTORS = 0;
      disp('cannot apply FFT scale factors when extracting a repetition due to lack of headers -- skipping scaling');
    end;
    ...
    if ( DO__CANONICAL_REORDER_COIL_CHANNELS && DO__EXTRACT_REPETITION ),
      DO__CANONICAL_REORDER_COIL_CHANNELS = 0;
      disp('cannot reorder channels when extracting a repetition due to lack of headers -- skipping reordering');
    end;
...
%  scan_patrefscan          = matrixtype([]);
  ...
  try,
    ...
    while ( ~ACQEND ),
      ...
      [mdh, scan_num] = read_meas_dat__mdh_binary(mdh_binary);
...
%      fseek(fp, fpos_rep, 'bof');
      ...
      if ( DO__EXTRACT_REPETITION && ((mdh(idx).ushRepetition+1) > iRep+Nmeas-1) ),
        break;
      end;
...
%              pos(10), pos(11), pos(12), pos(13), pos(14), pos(15), pos(16)) = mdh(idx).ulScanCounter;
	...
	%%% CATEGORY #4A: phase correction navigator line for data
	  ...
	  if ( ~FLAG__FOUND_EXTRACT_REPETITION )
	    continue;
	  end;
...
%      meas.scan_patrefscan = scan_patrefscan;
    ...
    if ( exist('read_meas_prot', 'file') && IS__VB13A_RELEASE_VERSION && ~DO__EXTRACT_REPETITION ),
...
% //##ModelId=3AFAAF7801CF
  ...
  %************************************************************************%
  %%% $Source: /space/padkeemao/1/users/jonnyreb/cvsjrp/PROJECTS/IMAGE_RECON/mrir_toolbox/read_meas_dat__fast.m,v $
  %%% Local Variables:
  %%% mode: Matlab
  %%% fill-column: 76
  %%% comment-column: 0
