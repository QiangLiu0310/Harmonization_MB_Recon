function [kernel_conv, kernel_corr] = mrir_array_GRAPPA_conv_kernel_v2(G)
...
% kawin setsompop, 2011/Aug/30
% modify so that the kernel does not shift the data line
%**************************************************************************%
  ...
  % Kawin Setsompop
  % 09/06/11
  % padd the convolution kernel so that convolution will not result in shifing the k-space data. 
  % This has not been tested for R > 2.....
  s = size(kernel_conv);
  if R1 ~= 1
      kernel_conv =  [ zeros([s(1), R1 ,s(3:end)]),kernel_conv   ];
      s = size(kernel_conv);
  end
  if R2 ~= 1
      kernel_conv =  [ zeros([s(1:2), R2,s(4:end)]),kernel_conv   ];
  end
  return;
