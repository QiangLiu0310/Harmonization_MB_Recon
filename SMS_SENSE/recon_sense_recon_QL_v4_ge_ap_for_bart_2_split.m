...
data_path='/data/pnlx/home/ql087/data_bwh/For_Hun/GE_Premier_Phantom_Feb_08_2025/Exam20407/Series3/'; % image
%% read GE MB2 data

cd(data_path);
disp('Currently in directory rawdata');
tmp=strcat(data_path,'ScanArchive_LONGWOOD30MR2_20250208_143151629.h5');
pfile = fullfile(tmp);
...
load('/data/pnlx/home/ql087/data_bwh/For_Hun/GE_Premier_Phantom_Feb_08_2025/Exam20407/Series3/SurfaceImages.mat')
...
kdata_dwi=zeros(146*2,146,size(img_k,3), size(img_k,4), size(img_k,5)/2);

for iReps =1:NRep/2 % for Hwihun: change it to 1:NRep/2 for the first part
    ...
    toc
    kdata_dwi(:,:,:,:,iReps)=kdata;% for Hwihun: change the 31 to 0 for first group
    disp(iReps)
end
...
for dwi_idx = 1:DWI
    ...
    writecfl(sprintf('kdata_%d', dwi_idx), kdata_slice); % for Hwihun: change the 31 to 0 for first group
end
