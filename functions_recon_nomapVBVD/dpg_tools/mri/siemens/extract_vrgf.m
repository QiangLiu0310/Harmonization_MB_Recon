function vrgf = extract_vrgf( filename,prot )
...
vrgf = regrid_epi_rampsamp( prot );%QL

return;

function vrgf = regrid_epi_rampsamp( prot ) %QL
...
% t_ft = t_ft(1);
t_ft=prot.alRegridFlattopTime; %QL
...
% t_ru = t_ru(1);
t_ru=prot.alRegridRampupTime;%QL
...
% t_rd = t_rd(1);
t_rd=prot.alRegridRampdownTime; %QL
...
% t_ds = t_ds(1);
t_ds=prot.alRegridDelaySamplesTime;
...
% inres = inres(1);
inres=prot.iNoOfFourierColumns; % QL, not sure
...
% outres = outres(1);
outres=prot.lBaseResolution;%QL
...
t_sw=prot.aflRegridADCDuration;
