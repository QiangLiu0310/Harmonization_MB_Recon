function [img] = recon_sms_std( varargin, prot )
%-------------------------------------
...
%
%Ncoils = hdr.ncoils;%QL
Ncoils=size(prot.sCoilElementID_tElement,2);
NRefLin = hdr.NRefLin;
hdr.NSlc=prot.sSliceArray_lSize;% QL
NSlc = hdr.NSlc; % QL

NRep = k.image.NRep;
hdr.R=prot.lAccelFactPE; % QL
...
% slice order QL: I think it was wrong to split them into 2 groups
slic_indexS=[1:NSlc];

% slic_indexS=reshape(slic_indexS,[NSlc/hdr.SMS, hdr.SMS])';
% slic_indexS=reshape(slic_indexS,[NSlc,1]);
[~,order_acs]=sort(slic_indexS);
...
% evp.NSlcMeas = length(hdr.slcindx); QL
evp.NSlcMeas=NSlc; %QL
...
%
v = extract_vrgf(k.image.filename,prot);         % build a VRGF conversion %QL
...
sz_refscanPC=[NFreq_outres  Ncoils  NLocPhz          NMultiplex*NSlc*2   NSeg]; % *2 debug QL
sz_refscan  =[NFreq_outres  Ncoils  NRefLin/NSeg     NMultiplex*NSlc*2   NSeg];% *2 debug QL
...
% refscanPC_raw_tmp=refscanPC_raw_tmp(:,:,1:size(refscanPC_raw_tmp,3)/2);% QL debug
% refscanPC_raw = reshape(v'*reshape(refscanPC_raw_tmp,[NFreq_inres Ncoils*NLocPhz*NMultiplex*NSlc*NSeg*2]), sz_refscanPC); % QL *2
refscanPC_raw = v'*reshape(refscanPC_raw_tmp,[NFreq_inres Ncoils*NLocPhz*NMultiplex*NSlc*NSeg*2]); % QL: not sure why there is a 2
...
% refscanPC_ROp_ROn(:,:,2,:,:,:) = squeeze(refscanPC_raw(:,:,2,:,:,:)); % it was fe/coil/2/slice

refscanPC_ROp_ROn=zeros(NFreq_outres,Ncoils,2,NSlc*2);
refscanPC_ROp_ROn(:,:,1,:)=squeeze((refscanPC_raw(:,:,1:3:end)+refscanPC_raw(:,:,3:3:end))/2);
refscanPC_ROp_ROn(:,:,2,:)=refscanPC_raw(:,:,2:3:end);
refscanPC_ROp_ROn=refscanPC_ROp_ROn(:,:,:,1:end/2);


%refscan: ACS data that can be used to train GRAPPA coefficients
refscan_raw_tmp = k.refscan.unsorted();

refscan_raw = reshape(v'*reshape(refscan_raw_tmp,[NFreq_inres Ncoils*NRefLin*NMultiplex*NSlc*2]), sz_refscan);
refscan_raw=refscan_raw(:,:,:,1:end/2);
...
% QL: the order is not correct by checking the images
tmp111=[1:2:NSlc 2:2:NSlc];
[~,orr]=sort(tmp111);
C2=C2(:,:,orr,:);
tmp=[2:2:NSlc;1:2:NSlc];
tmp=reshape(tmp,[NSlc,1]);
[~,orr]=sort(tmp);
C2=C2(:,:,orr,:);
clear tmp tmp111
