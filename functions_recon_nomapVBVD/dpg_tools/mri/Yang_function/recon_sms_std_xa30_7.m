function [image] = recon_sms_std_xa30_7( varargin, prot,save_path )
%% gSlider
twix = varargin;
if iscell(twix)
    k = twix{length(twix)};
...
[dimY, dimX, dimZ, dimW, dimV] = size(k_all); % pe fe slice coil diffusion

img_aa = zeros(dimX, dimX, dimW, dimV);  
image = zeros(dimX, dimX, dimW, dimV, dimZ); 

for cntsli = 1:dimZ
    for cntcoil = 1:dimW
        for cRep = 1:dimV
            img_aa(:,:,cntcoil,cRep) = flip(reconhd(k_all(:,:,cntsli,cntcoil,cRep), dimY, dimX));
            image(:,:,cntcoil,cRep, cntsli) = fif(img_aa(:,:,cntcoil,cRep)); % do a fif to the kdata
        end
    end
end
% img(:,:,cntsli,:) = sqrt(sum(abs(fif(img_aa)).^2, 3)); 

image= squeeze(sqrt(sum(abs(image).^2, 3))); 